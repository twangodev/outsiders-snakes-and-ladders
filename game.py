...
grid = pygame.image.load("./assets/grid-snakes.png")
...
def snakes_and_ladders(position, offset):
    
    #Snakes
    alert_text = font.render("You discovered and took a snake!", True, (0,0,0))
    if position == [6-1, 0]:
        newposition = [6-1, 4]
        newoffset = [(newposition[0]*60)+config["snake-xoffset"], newposition[1]*60]
        return newposition, newoffset, alert_text
    elif position == [3-1, 1]:
        newposition = [9-1, 6]
        newoffset = [(newposition[0]*60)+config["snake-xoffset"], newposition[1]*60]
        return newposition, newoffset, alert_text
        ...
        return newposition, newoffset, alert_text
    #Ladders
    alert_text = font.render("You discovered and took a ladder!", True, (0,0,0))
    if position == [1-1, 3]:
        newposition = [2-1, 0]
        newoffset = [(newposition[0]*60)+config["snake-xoffset"], newposition[1]*60]
        return newposition, newoffset, alert_text
    elif position == [10-1, 5]:
        newposition = [9-1, 2]
        newoffset = [(newposition[0]*60)+config["snake-xoffset"], newposition[1]*60]
        return newposition, newoffset, alert_text
    elif position == [4-1, 7]:
        ...
        alert_text = None
        return position, offset, alert_text
#Main Modules
        ...
        if bannerdisplay == True:
            ...
            if correct_checker(id_randomized[0], provided_answer) == True:
                ...
                if stoprolling == False:
                    ...
                    position, offset, alert_text = snakes_and_ladders(position, offset)
                    stoprolling = True
                ...
                #Snake Ladder Checker
                if alert_text != None:
                    gameDisplay.blit(alert_text, [int(config["display"]["width"]/2-180), int(config["display"]["height"]/2)])
                #Render Buttons
